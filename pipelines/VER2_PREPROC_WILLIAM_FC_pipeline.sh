...
LVL1_REST_MERGE_RESIDUALS="no"
LVL1_DETREND_BANDPASS_RESIDUALS="no"
LVL1_REST_DETREND_BANDPASS_RESIDUALS="no"
...
if [[ "$SUBJ_LEVEL_PREPROC" == "yes" ]]; then
	...
	for subj in "${DATA_ARR[@]}"; do
		...
		if [[ "${REALIGN_RESLICE}" == "yes" ]]; then
			...
			spm_realicgn_reslice.sh ${EPI_DATA_LIST} ${REALIGN_TO_MEAN} ${subj}_spm_realign_reslice 1
			rm -rf ${EPI_DATA_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT1/${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_realign_reslice.sh ${EPI_DATA_LIST} ${REALIGN_TO_MEAN} ${subj}_spm_realign_reslice 1
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT2/${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_realign_reslice.sh ${EPI_DATA_LIST} ${REALIGN_TO_MEAN} ${subj}_spm_realign_reslice 1
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT3/${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_realign_reslice.sh ${EPI_DATA_LIST} ${REALIGN_TO_MEAN} ${subj}_spm_realign_reslice 1
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT4/${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_realign_reslice.sh ${EPI_DATA_LIST} ${REALIGN_TO_MEAN} ${subj}_spm_realign_reslice 1
			rm -rf ${EPI_DATA_LIST}			
		...
		if [[ "${COREG_EPI_T2}" == "yes" ]]; then
			...
			arEPI_DATA_VOL=$(ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT1/ar*.nii | sed -n '1p')
			T2_DATA_VOL=$(ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/T2/${T2_FILENAME_GLOB})
			spm_coreg ${T2_DATA_VOL} ${arEPI_DATA_VOL} ${COREG_OTHER} ${subj}_spm_coreg_T2-EPI 1
			arEPI_DATA_VOL=$(ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT2/ar*.nii | sed -n '1p')
			echo "arEPI_DATA_VOL : ${arEPI_DATA_VOL}"
			spm_coreg ${T2_DATA_VOL} ${arEPI_DATA_VOL} ${COREG_OTHER} ${subj}_spm_coreg_T2-EPI 1
			arEPI_DATA_VOL=$(ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT3/ar*.nii | sed -n '1p')
			echo "arEPI_DATA_VOL : ${arEPI_DATA_VOL}"
			spm_coreg ${T2_DATA_VOL} ${arEPI_DATA_VOL} ${COREG_OTHER} ${subj}_spm_coreg_T2-EPI 1		
			arEPI_DATA_VOL=$(ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT4/ar*.nii | sed -n '1p')
			echo "arEPI_DATA_VOL : ${arEPI_DATA_VOL}"
			spm_coreg ${T2_DATA_VOL} ${arEPI_DATA_VOL} ${COREG_OTHER} ${subj}_spm_coreg_T2-EPI 1	
		...
		if [[ "${SEGMENT_T1}" == "yes" ]]; then
			...
			T1_DATA_VOL=$(ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/T1/${T1_FILENAME_GLOB})
		...
		if [[ "${DENOISE_EPI}" == "yes" ]]; then
			...
			rm -rf ${EPI_DATA_LIST} 
			rm -rf ${DERIVATIVES_DIR}/${subj}/nii/ses01/VRIT1/REG_LIST.txt
			#read -p "請確認上述路徑正確，按 Enter 開始掃描..." < /dev/tty
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses01/VRIT1/rp_*.txt >> ${REG_LIST}

			
			#####################
			echo "EPI_DATA_LIST:${EPI_DATA_LIST}"
			echo "REG_LIST:${REG_LIST}"
			echo "SLICE_TIME_REF_SLICE:${SLICE_TIME_REF_SLICE}"			
			echo "WM_MASK:${WM_MASK}"			
			echo "CSF_MASK:${CSF_MASK}"			
			echo "DENOISE_MASK_THRESHOLD:${DENOISE_MASK_THRESHOLD}"			
			echo "DENOISE_CENSOR_METHOD:${DENOISE_CENSOR_METHOD}"
			echo "DENOISE_CENSOR_THRESHOLD:${DENOISE_CENSOR_THRESHOLD}"	
			#read -p "請確認上述路徑正確，按 Enter 開始掃描..." < /dev/tty
			####################
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses01/VRIT3/rp*.txt >> ${REG_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses01/VRIT4/rp*.txt >> ${REG_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT1/rp*.txt >> ${REG_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT2/rp*.txt >> ${REG_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT3/rp*.txt >> ${REG_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT4/rp*.txt >> ${REG_LIST}
...
if [[ "$GROUP_LEVEL_PREPROC" == "yes" ]]; then
	...
	if [[ "${MAKE_SST}" == "yes" ]]; then
		...
		for IMAGE in "${rc_GLOBS[@]}"; do
			...
			# for subj in "${DATA_ARR[@]}"; do
				#read -p "請確認上述路徑正確，按 Enter 開始掃描..." < /dev/tty
			ls -1 ${DERIVATIVES_DIR}/RCS/SES01/$IMAGE*.nii >> ${IMAGE}_LIST.txt
			ls -1 ${DERIVATIVES_DIR}/RCS/SES02/$IMAGE*.nii >> ${IMAGE}_LIST.txt
