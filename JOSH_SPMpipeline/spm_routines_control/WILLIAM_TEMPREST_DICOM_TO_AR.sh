...
if [[ "$SUBJ_LEVEL_PREPROC" == "yes" ]]; then
	...
	for subj in "${DATA_ARR[@]}"; do
		...
		if [[ "${DICOM2NII}" == "yes" ]]; then
			echo "Working on ${subj} DICOM conversion."
			mkdir -p ${PROJECT_DIR}/data/rawdata/$subj/nii/ses01/REST_SES01

			dcm2niix -o ${PROJECT_DIR}/data/rawdata/$subj/nii/ses01/REST_SES01 ${PROJECT_DIR}/data/sourcedata/${subj}/ses01/brain/mri/func/REST
			cp -rf ${PROJECT_DIR}/data/rawdata/$subj/nii/ses01/REST_SES01 ${DERIVATIVES_DIR}/$subj/nii/ses01
		...
		if [[ "${REALIGN_RESLICE}" == "yes" ]]; then
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses01/REST_SES01/${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
		...
		if [[ "${SLICE_TIME_CORRECTION}" == "yes" ]]; then
			...
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses01/REST_SES01/r${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
