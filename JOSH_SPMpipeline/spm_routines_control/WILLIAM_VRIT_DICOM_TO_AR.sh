...
#SEL_DATA_ARR=("o15")
DATA_ARR_FULLPATH=(${PROJECT_DIR}/data/derivatives/*) ###############################
...
if [[ "$SUBJ_LEVEL_PREPROC" == "yes" ]]; then
	...
	for subj in "${DATA_ARR[@]}"; do
		...
		if [[ "${SLICE_TIME_CORRECTION}" == "yes" ]]; then
			...
			spm_slice-timing.sh ${EPI_DATA_LIST} ${SLICE_TIME_REF_SLICE} ${SLICE_TIME_SLICE_ORDER} ${subj}_spm_slice-timing 1
			rm -rf ${EPI_DATA_LIST}
			...
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT1/r${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_slice-timing.sh ${EPI_DATA_LIST} ${SLICE_TIME_REF_SLICE} ${SLICE_TIME_SLICE_ORDER} ${subj}_spm_slice-timing 1
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT2/r${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_slice-timing.sh ${EPI_DATA_LIST} ${SLICE_TIME_REF_SLICE} ${SLICE_TIME_SLICE_ORDER} ${subj}_spm_slice-timing 1
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT3/r${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_slice-timing.sh ${EPI_DATA_LIST} ${SLICE_TIME_REF_SLICE} ${SLICE_TIME_SLICE_ORDER} ${subj}_spm_slice-timing 1
			rm -rf ${EPI_DATA_LIST}
			ls -1 ${DERIVATIVES_DIR}/${subj}/nii/ses02/VRIT4/r${EPI_FILENAME_GLOB}.nii >> ${EPI_DATA_LIST}
			spm_slice-timing.sh ${EPI_DATA_LIST} ${SLICE_TIME_REF_SLICE} ${SLICE_TIME_SLICE_ORDER} ${subj}_spm_slice-timing 1
			rm -rf ${EPI_DATA_LIST}
